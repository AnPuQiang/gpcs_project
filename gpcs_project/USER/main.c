...
//	memcpy(share_resource,task1_str,sizeof(task1_str)); //Ïò¹²Ïí×ÊÔ´Çø¿½±´Êı¾İ
	...
	while(1)
	{
		fail_filesize = 0;	//Ã¿´ÎÑ­»·¶¼¶Ô·¢ËÍÊ§°Ü´¦µÄ±ê¼Ç½øĞĞ³õÊ¼»¯
		USART2_Init(115200);
...
//		printf("%s\r\n",share_resource);	//´®¿ÚÊä³ö¹²Ïí×ÊÔ´ÇøÊı¾İ	
//		OSSemPost (&MY_SEM,OS_OPT_POST_1,&err);				//·¢ËÍĞÅºÅÁ¿
...
//	f_printf(file,"\r\n");
	printf("Ğ´ÈëÎÄ¼ş·µ»Ø´úÂë£º%d\r\n",res_sd);	
	res_sd=f_close(file);
	printf("¹Ø±ÕÎÄ¼ş·µ»Ø´úÂë£º%d\r\n",res_sd);
}
